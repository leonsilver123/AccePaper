...
// the `SessionEventMap` `declare module` augmentation that wires the three
// `research/*` whole-value events into the core session event vocabulary lives
// HERE (mirroring agent-team `types.ts`). This file is also the one runtime
// validation seam: every payload that reaches the durable Lead log — and every
// payload folded by the host projection — passes one of the exported zod
// schemas below, so a corrupt/legacy payload fails loudly at the log boundary
// instead of silently poisoning the fold.
//
// Event-wire shapes (§2 of the frozen design spec):
...
/** The three research-domain event discriminants injected into `SessionEventMap`. */
export type ResearchEventType = 'research/member' | 'research/task' | 'research/rev'
